...
export namespace ProcessName {
  const nameMap: { [key in ProcessName]: string } = {
    ...
    [ProcessName.DailycomparNative]: '每日回比本命',
    [ProcessName.NativecomparDaily]: '本命比每日回',
  ...
  const pathMap: { [key in ProcessName]: Path } = {
    ...
    [ProcessName.DailyReturn]: Path.DailyReturn,
    ...
    [ProcessName.DailycomparNative]: Path.DailycomparNative,
